...
#define TELEMETRY_PROP_NAME_POSICION "posicion"
    ...
    size_t* payload_buffer_length)
{
  ...
  az_span json_span;
  float temperature, humidity, gas, pressure, altitude;
  ...
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_POSICION));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding posicion property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, posicion, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding posicion property value to telemetry payload.");
